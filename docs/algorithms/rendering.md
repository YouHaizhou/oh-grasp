...
**位置**：render.js:106（`render`）、render.js:129（CLI）。
...
**三个清单的行长一个样，因为它们就是同一个函数**：端口清单、消费者清单、以及 group 弹窗第二段「成员直连外部」的行 markup 都由 `rowHtml(src, lab, dst)`（viewer.js:566）产出，三个参数都收**已转义**的字符串。ADR-0011 的「学一次行语法，用在两处」不是一个说法，是这一个函数（票 06 起是**三**处）；票 05 第二片把「清单行可点」做出来时，改一处就够。
...
**语言怎么进去**：语言**不是**内核状态，而是参数。`pick(field, lang)` 收口全部散文字段（`{zh, en}` 按语言取值，旧形态的普通字符串原样返回）——**对通过校验的 IR 而言**这条回退分支不可达：契约步之后校验硬性要求两个语言都在（见 `validation.md` §4），走到这里的一定是 `{zh, en}`；但 `viewer.js` 是**纯内核**，它照收任何形态的 IR——`oh-grasp/test/layout.test.js` 就正把单语字符串、`{zh: …}`、`[{zh: …}]` 直接喂进 `pick` / `pickList` 并断言这条回退生效，**所以它在内核层是活的，不要当死代码删掉**。它和四段的读取口 `pickPart`（回退阶梯同款：当前语言 → `zh` → `en`）是 **expand 期的脚手架**，契约步保留不删；**若将来有人放宽了双语的硬校验，这两处就是中英混杂回到产物里的入口，必须同时删掉**。`edgeLabel(e, lang)`、`nodeSvg(id, x, y, counts, lang, M, G)`、`flowSvg(ids, edges, counts, lang)`、`portListHtml(id, dir, edges, lang, M, G)`、`fourPartHtml(edge, lang, M)` 逐层把它传下去。`nodeSvg` / `portListHtml` / `fourPartHtml` 额外收模型索引 `M` / `G`（内核不持有模型，测试才好直接喂数据）。DOM 应用把当前语言放在 `state.lang`（不持久化），点 header 的 `#langZh` / `#langEn` 就换值并整页重渲染。`T = {zh, en}` 是查看器固定文案表（输入/输出/依赖/内部数据流/组间边/成员直连外部/宿主/叶子依赖行的两个标头 `dep` 与 `host`/GROUP/INTERNAL/EXTERNAL/空方向「—」/连线/四段说明/四段字段名/消费者/消费者空清单一句/提示行），`tr(lang, key)` 取它，`fmt(tpl, n)` 填 `{n}` 占位——它在内核里，因此「两种语言的键一一对应」可单测。**分节标题与行内标头是两个键**：`deps` 的值是双语的「依赖 Dependencies」（侧栏分节标题的写法），行内那一行用 `dep`（「依赖」/「Dependency」），否则会读出「依赖 Dependencies fs · …」。
...
**位置**：viewer.js:849（分界）、viewer.js:1365（导出）。
...
| 线 | 位置 | 干什么 |
|---|---|---|
| 入口描述（内容形状） | DOM 门内，`renderA` 里的 `detailSpec` / `groupSpec` / `portSpec` / `edgeSpec` / `consSpec`（viewer.js:1160 起） | 各产出 `{ name, kind, html, cls, mount }`：标题与 kind 是**未转义原文**，`html` 是内容，`cls` 是三档宽度（`''` / `vA-modal-wide` / `vA-modal-sm`），`mount` 是「塞进去之后才做得了的事」（子图的缩放控制器、节点选中态、网格卡片与子图节点的点击） |
| 容器与关闭 | DOM 门内 `openModal(spec)`（viewer.js:1146） | 拼唯一一份外壳（标题 / kind 标签 / `×`）、`.classList.add('open')`、**唯一一处** `#detailClose` 绑定，然后调 `spec.mount()`。四个入口因此都是**一行**：`openDetail` / `openGroup` / `openPort` / `openEdge` / `openConsumers`（viewer.js:1269-1273） |
| 关闭槽位 | 门内的槽位（但**不在** `renderA` 内） | `closeOverlay`（viewer.js:861）由每次 `renderA` 末尾刷成新的 `closeDetail`（viewer.js:1129）；`document` keydown 监听器在门的自启动 IIFE 里**只注册一次**（不像 `bindEdges` 那样每次 `renderA` 重挂），按 `Esc` 就调 `closeOverlay()`（viewer.js:1356） |

为什么绕这么一圈：`document` 上的 keydown 不能每次重渲染都加一个（切十次语言就有十个监听器），而 `closeDetail` 又必须是最新那棵树上的函数——一个可变槽位是两头都能满足的最小写法。三条关闭路径都落在同一个 `closeDetail` 上：`×`（openModal 里那一处绑定）、点遮罩空白（viewer.js:1319，`e.target === detail`）、`Esc`（槽位）。

**第四个入口的绑定**：`root.querySelectorAll('.vA-extcard')` → `openConsumers(id)`（viewer.js:1336）。卡片是 `<button type="button">`（可聚焦，与 `.vA-mcard` 同一个理由），`data-id` 带的是 external 模块 id；它不在画布里，所以没有拖拽阈值这回事。
...
**拖拽阈值同样适用**：`bindEdges`（viewer.js:1278）先看 `controller.moved`（见 viewport.md 的 3px 阈值）——拖动画布时顺手扫过一条边，不该弹出四段。节点与端口上的 `click` 也照此办。
