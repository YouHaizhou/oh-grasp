...
**位置**：viewer.js:261（后向边判定，在 `flowGeometry` 内）。
...
**兜底分支**：理论上 DAG 跑完 Kahn 所有节点都会出队。代码里仍留了「未出队的节点放到最深层 +1」的兜底（viewer.js:289），因为上游的断环若有疏漏（例如自环 `from === to`），Kahn 会静默漏掉节点而不是报错——兜底保证**任何输入都不丢节点**。图上一个位置错了的盒子，好过一个凭空消失的盒子。

**最长路分层允许一条边跨多层**：`layer[v]` 取的是**最深**前驱 +1，所以 `u → v` 完全可能跨过 2 层以上（`to.layer − from.layer ≥ 2`）。层号因此有两个用处：定纵向坐标，以及**判定这条边走哪种形状**——跨层 ≥2 的走下节的虚节点正交折线（geometry.md §1）。`flowGeometry` 里这份判定就在分层之后、层内序列之前。
...
**复杂度**：O(V+E)，受上述两处影响。
**位置**：viewer.js:283（分层那一行，在 `flowGeometry` 内）。
...
**虚节点一起参与排序**（ADR-0009）：跨层 ≥2 的前向边被拆成「真实源 → 虚节点 → … → 真实目标」的**逐层链**，虚节点进层内序列、与真实节点用同一套权重排序。排序用的邻接因此是**真实前向边 + 这些链 hop**（代码里的 `baryEdges`）；虚节点被上下两跳同时牵引，最终落到「既靠近源、又靠近目标」的那条空隙里——它宽 0，落点必然是空隙（geometry.md §1）。返回给渲染的 `fwd` 仍是**真实边**，不受这份邻接影响；`layers` / `pos` 里则确实多了这些无实体单元，所以要按 `virtual` 表过滤才是「盒的清单」（测试：「virtual nodes take a slot in the layer order but carry no box」）。

**为什么用重心法**：它是交叉最小化的经典启发式（也是最常用的基线），单遍线性、实现十几行。精确最优同样是 NP-hard。代价是可能收敛到局部最优——但对「层数 ≤5、每层 ≤4 个盒子」的实际图形，局部最优和全局最优基本一致。

**已知低效**：`predsOf`/`succsOf` 每次调用都 `baryEdges.filter(...)` 扫一遍全边集 → 单次排序 O(V·E)，整体 O(V·E·passes)。修法是预建 `from → edges` / `to → edges` 邻接索引（断环那一步已经有 `adj` 了，复用即可）。**虚节点把 E 撑宽了**（|E| += Σ(跨度−1)），这条负债因此比 ADR-0012 记它的时候更明显——量级仍然无碍。

**复杂度**：O(V·E·passes)，E 含虚节点链的 hop。
**位置**：viewer.js:329（层内排序 / 重心扫描，在 `flowGeometry` 内）。
**测试**：层内顺序不影响 `flowGeometry` 的接口契约（`layers` 数组本身是返回值，测试断的是分层正确性与不重叠）；虚节点的占位与排序另见「virtual nodes take a slot in the layer order but carry no box」。
...
**单位：`countPorts` 数的是「单元 id × 方向」下的**内容种数**（票 03 确认）。**不是连接条数（去重闸门就是为此），**也不是画布上的几何段数**——一条跨层 ≥2 的前向边在几何上被折成好几段，那些段与端口计数毫无关系。这条区分在虚节点落地后变得要紧：**虚节点绝不能成为一个计数单元**（它没有盒、没有端口，一旦进来就凭空多出一个 `×N`）。它只活在 `flowGeometry` 的几何里、不进连接表，所以 `countPorts` 天然看不到它——改 `flowGeometry` 时别把虚节点塞进 aggregated edges。测试：「virtual nodes take a slot in the layer order but carry no box」（断言 `counts['__vn*']` 是 `undefined`）。
...
**复杂度**：O(E)。
**位置**：viewer.js:153（`countPorts`）、viewer.js:545（`portRows`）、viewer.js:573（`portListHtml`）。
...
**单位：`edgeLabel` 不计数（票 03 确认）。** 它产出的是**一段文本**（≤3 个用 ` · ` 连、再多收尾成 `+N`），`+N` 里的 N 是「还有 N 种**不同标签**」——不是连接条数，也不是画布上的几何段数（长边的折线段数由 `flowGeometry` 决定，与此无关）。

**复杂度**：O(n²)（`indexOf` 查重），n = 该边折的 connection 数（≤12），无碍。
**位置**：viewer.js:185（`uniqJoin`）、viewer.js:199（`edgeLabel`）。
...
**同向自环**：折叠后 `from === to` 的边（组内边在顶层视图下）**不进顶层**，由调用方过滤（viewer.js:1101，`renderA` 里折叠时就地丢掉）。

**单位：一条聚合边（票 03 确认）。** 入参是 IR 的 connections，出参的条数 = 同向 `(from,to)` 对的个数——**不是 connection 条数，也不是几何段数**。虚节点不进这里：它没有 connection；一条长边在画布上被折成几段是 `flowGeometry` 那份折点几何量（`longs`）的事，与聚合无关。

**复杂度**：O(E)。
**位置**：viewer.js:210（`aggregateEdges`）。
...
字段名固定，**不得改叫 `origin` / `how` / `what` / `why`**：改名会丢掉字段名与用户原话的一一对应；`from` 已被 connection 占用表示源模块 id，也挪不过来。**写在每条 connection 上，不建字典**——这样任何出现位置（顶层聚合边、子图、端口清单）点开都一致。四段是**必填**字段：中英两个语言子树、每个子树四个格子，缺一格硬报错（契约步已落地，见 `validation.md` §5）。

**三个函数各管一段**（照 `portRows` / `portListHtml` 的分家方式）

| 函数 | 干什么 |
|---|---|
| `pickPart(desc, lang, key)` | 取四段里的**一段**：先当前语言、再默认语言、再另一种语言。缺当前语言的回退与 `pick` 同源，是 expand 期的脚手架——契约步的双语硬校验**已上线**，**对通过校验的 IR** 两个语言都在，这条回退不可达；但内核层仍活（`layout.test.js` 直接把只带一种语言的四段喂进来并断言回退），保留不删，理由见 `rendering.md` §3 |
...
**缺 description 时每段显式画「—」**：`generated/` 里那份**旧**产物 47 条 connection 一条 description 都没有（重新生成由用户手动做，是契约步交付之后的事），点开必须不抛错、也不能静默留白——留白会让人以为那条线没有用途。流向那一行照常显示 label。

**清单行故意不可点**：「点开某一条 connection、再退回上一层」属于弹窗合并那一票，不在本步。此处只把四段平铺出来。

**复杂度**：O(E)，E = 该边的 connection 数。
**位置**：viewer.js:705（`pickPart`）、viewer.js:718（`fourPartRows`）、viewer.js:731（`modName`）、viewer.js:739（`fourPartHtml`）。
