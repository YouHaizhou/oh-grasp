...
- [ ] **过渡期容忍，不单独开 ADR**：渲染侧的 `pick` 在缺当前语言时会回退到另一语言（`viewer.js:76-77`），这正是 ADR-0008 明确否掉的「静默回退」形态。它是 expand 期的脚手架——「双语必填」上线后，通过校验的 IR 两个语言都在，该分支**不可达**。本票不删它，只在此记录：**若将来有人放宽了硬校验，这两行就是中英混杂回到产物里的入口，必须同时删掉。**
