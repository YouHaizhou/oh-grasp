...
- [ ] **过渡期容忍，不单独开 ADR**：渲染侧有**两处**按语言取值的地方会在缺当前语言时回退到另一语言，这正是 ADR-0008 明确否掉的「静默回退」形态。它们是 expand 期的脚手架——「双语必填」上线后，通过校验的 IR 两个语言都在，这两条分支**不可达**。本票不删它们，只在此记录：**若将来有人放宽了硬校验，这两处就是中英混杂回到产物里的入口，必须同时删掉。**
  ...
  - `pickPart`（`viewer.js:707-711`）—— **四段连接描述的读取口**。它和 `pick` 同类、回退阶梯也一样（当前语言 → `zh` → `en`），但更容易被漏掉：`connection.description` 的四段正是本票**另一个硬校验对象**，而四段是产物里最长的一段散文。`viewer.js:703-704` 的注释其实自己写了「与票 08 对 pick 的记录同源」，`cf04035` 的提交信息也记了这处，是票文件漏登。
